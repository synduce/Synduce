...
let rec no_bool = function
  | Program stmts -> no_bool_stmts stmts
  | EmptyProgram -> true
...
and no_bool_const = function
  | ConstantBool b -> false
  | ConstantInt i -> true
...
and aux_stmt = function
  | While (a, b) ->
    if aux_expr a = 1 && aux_type_expr a = 1 && aux_stmt b = 1 then 1 else 0
...
and aux_expr = function
  ...
  | BinaryExpression (o, l, r) ->
    if (aux_type_binop_operand o = 3 && aux_type_expr l = aux_type_expr r)
       || (aux_type_expr l > 0
           && aux_type_expr r > 0
  ...
  | UnaryExpression (o, e) ->
    if aux_type_expr e = 3 || (aux_type_expr e > 0 && aux_type_unop o = aux_type_expr e)
    then 1
    else 0
  | If (c, l, r) ->
    if aux_type_expr c = 1 && aux_type_expr l = aux_type_expr r then 1 else 0
...
and aux_type_expr = function
  ...
  | UnaryExpression (o, e) ->
    if aux_expr (UnaryExpression (o, e)) > 0 then aux_type_unop o else 0
  | If (c, l, r) ->
    if aux_expr (If (c, l, r)) > 0
    then if aux_type_expr c = 1 then aux_type_expr l else 0
    else 0
...
and aux_type_unop = function
  | Minus -> 2
  | Not -> 1

and aux_type_const = function
  | ConstantBool b -> 1
  | ConstantInt i -> 2
;;
