...
type unique_list = AElt of int * pos | ACons of int * pos * unique_list
...
let rec repr = function AElt (k, v) -> dup k v | ACons (hdk, hdv, tl) -> dec hdk tl hdv

and dec k tl = function S v -> Cons (k, dec k tl v) | One -> Cons (k, repr tl)
