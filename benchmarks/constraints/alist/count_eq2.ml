...
type unique_list = ANil | ACons of int * nat * unique_list
...
let rec repr = function ANil -> Nil | ACons (hdk, hdv, tl) -> dec hdk tl hdv

and dec k tl = function S v -> Cons (k, dec k tl v) | Z -> repr tl
...
let target a l =
  ...
  and int_of = function S n -> 1 + int_of n | Z -> 0 in
  g l
  [@@requires is_alist]
