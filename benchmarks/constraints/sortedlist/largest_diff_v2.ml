...
let rec is_sorted = function Elt x -> true | Cons (hd, tl) -> aux hd tl
...
let rec is_concat_sorted l = (fun (x, y, z) -> x) (is_concat_sorted_aux l)

and is_concat_sorted_aux = function
  | Single a -> (a, a, true)
  | Concat (y, z) ->
      (fun (a, b, c) (a2, b2, c2) -> (a, b2, (a2 > b && c) && c2))
        (is_concat_sorted_aux y) (is_concat_sorted_aux z)
...
let rec amax = function Single x -> [%synt g0] x | Concat (x, y) -> [%synt join] (amax x) (amax y)
