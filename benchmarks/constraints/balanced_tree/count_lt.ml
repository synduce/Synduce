...
let rec bst_balanced = function
  | Leaf a -> true
  | Node (a, b, c) ->
      a >= tree_max b && a <= tree_min c && aux b = aux c && bst_balanced b && bst_balanced c

and aux = function Leaf x -> 0 | Node (a, b, c) -> 1 + max (aux b) (aux c)
...
let target y t =
  ...
  and size = function Leaf a -> 1 | Node (a, l, r) -> 1 + size l + size r in
  g t
