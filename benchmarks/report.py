...
def save_scatter_plot_table2(scatter_file, se2gis_series, segis_series, timeouts=False):
    ...
    if timeouts:
        ...
        ax.set(xlim=(ax_min, ax_max),
               ylim=(ax_min, ax_max))
        ax.set_xlabel(f"Synthesis time using {kw_segis_short} (log)",
                      fontsize=plot_fontsize)
        ax.set_ylabel(f"Synthesis time using {kw_tool_main_algo} (log)",
                      fontsize=plot_fontsize)
    ...
    with open(input_file, 'r') as csv:
        for line in csv.readlines():
            ...
            if len(info) >= 15:
                ...
                table2[benchmark] = (se2gis_result, segis_result, cegis_result)
