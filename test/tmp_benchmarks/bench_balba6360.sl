(set-logic DTLIA)
(declare-datatype synd_tup_Int_Int_Bool
 ((mk_synd_tup_Int_Int_Bool (proj_synd_tup_Int_Int_Bool_0 Int) (proj_synd_tup_Int_Int_Bool_1 Int)
   (proj_synd_tup_Int_Int_Bool_2 Bool))))
(define-fun min ((x Int) (y Int)) Int (ite (<= x y) x y))
(synth-fun odot$2 ((x3 synd_tup_Int_Int_Bool) (x4 synd_tup_Int_Int_Bool)) Bool
 ((IStart Bool) (Ipred Bool) (Ix Int) (Ic Int))
 ((IStart Bool ((and Ipred Ipred)))
  (Ipred Bool
   ((proj_synd_tup_Int_Int_Bool_2 x3) (proj_synd_tup_Int_Int_Bool_2 x4) 
    (not Ipred) (and Ipred Ipred) (or Ipred Ipred) (= Ix Ix) (> Ix Ix) 
    (>= Ix Ix)))
  (Ix Int
   (Ic (proj_synd_tup_Int_Int_Bool_0 x3) (proj_synd_tup_Int_Int_Bool_1 x3)
    (proj_synd_tup_Int_Int_Bool_0 x4) (proj_synd_tup_Int_Int_Bool_1 x4) 
    (- Ix) (+ Ix Ix) (min Ix Ix) (ite Ipred Ix Ix)))
  (Ic Int ((Constant Int)))))
(declare-var b0 Bool)
(declare-var b1 Bool)
(declare-var i0 Int)
(declare-var i Int)
(constraint
 (or (not (and (<= i0 i) (and (<= i0 0) (= b1 (>= i0 0)))))
  (= b1 (odot$2 (mk_synd_tup_Int_Int_Bool 0 0 true) (mk_synd_tup_Int_Int_Bool i i0 b1)))))
(constraint
 (or (not (and (<= i0 i) (and (<= i0 0) (= b1 (>= i0 0)))))
  (= (and b1 (>= (ite b0 (+ i 1) (- i 1)) 0))
   (odot$2
    (mk_synd_tup_Int_Int_Bool (ite b0 (+ 0 1) (- 0 1)) (min 0 (ite b0 (+ 0 1) (- 0 1)))
     (and true (>= (ite b0 (+ 0 1) (- 0 1)) 0)))
    (mk_synd_tup_Int_Int_Bool i i0 b1)))))
(check-synth)
