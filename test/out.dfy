...
function rep(t : tree) : tree  {t}
function f(s : int, t : tree) : int decreases t
  {match t
  case nil => s
  case node(a, l, r) =>
      var sum := f(s, l);
      f(sum+a, r)}
function spec(t : tree) : int decreases t {f(0, t)}
...
lemma correctness_lemma(t : tree)  ensures (target(t) == spec(rep(t)))
  {match t
  case nil => {}
  case node(a, l, r) =>
  {
