...
let get_ensures (v : variable) : term option =
  match Hashtbl.find specs v.vid with
...
let pp_spec (frmt : Formatter.t) (s : spec) =
  ...
  | Some req ->
    Fmt.(pf frmt "@[%a %a@]" (styled (`Fg spec_color) string) "requires" pp_term req)
  | None -> ());
  ...
  | Some ens ->
    Fmt.(pf frmt "@[%a %a@]" (styled (`Fg spec_color) string) "ensures" pp_term ens)
  | None -> ());
  if is_not_empty s then Fmt.(pf frmt "%a" (styled (`Fg spec_color) string) "»")
;;
