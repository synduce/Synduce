...
let smt_of_aux_ensures ~(p : psi_def) : S.smtTerm list =
  let mk_sort maybe_rtype =
    match maybe_rtype with None -> S.mk_int_sort | Some rtype -> SmtInterface.sort_of_rtype rtype
  in
  ...
  let%lwt () =
    Lwt_list.iter_p
      (fun x ->
        ...
        return ())
      ((match p.psi_tinv with None -> [] | Some tinv -> SmtInterface.smt_of_pmrs tinv)
      @ (if p.psi_repr_is_identity then SmtInterface.smt_of_pmrs p.psi_reference
        else SmtInterface.smt_of_pmrs p.psi_reference @ SmtInterface.smt_of_pmrs p.psi_repr)
  ...
  let task (solver, starter) =
    ...
    let rec expand_loop u =
      ...
      | Some t0, true -> (
          ...
          | _, Some model ->
              Log.debug_msg
                "Bounded ensures verification has found a counterexample to the ensures candidate.";
              return model
