...
val interactive_add_lifting_expression :
  p:AState.psi_def -> AState.lifting -> Term.term -> int -> AState.lifting * Term.term option

val replace_boxed_expressions : p:AState.psi_def -> AState.lifting -> Term.term -> Term.term
...
val deduce_lifting_expressions :
  p:AState.psi_def -> AState.lifting -> Term.term option -> Term.term -> Term.term -> AState.lifting

val scalar :
  p:AState.psi_def ->
  AState.refinement_loop_state ->
  'a ->
  (AState.psi_def * AState.refinement_loop_state, Syguslib.Sygus.solver_response) result
