  ...
  *)
  let solve_constant_eqns (unknowns : VarSet.t) (eqns : equation list) =
    ...
    if List.length partial_soln > 0 then
      Log.debug
        Fmt.(fun fmt () -> pf fmt "@[Constant:@;@[<hov 2>%a@]@]" pp_partial_soln partial_soln);
  ...
  let core_solve ?(predict_constants = None) ~(gen_only : bool) (unknowns : VarSet.t)
    ...
    if not gen_only then
      ...
      ( Lwt.map (function Some resp -> handle_response resp | None -> (RFail, Either.Second [])) t,
        r )
...
let solve ~(p : psi_def) (eqns : equation list) :
    ...
    let unknowns', eqns', postprocessing_actions =
      List.fold preprocessing_actions ~init:(unknowns, eqns, [])
        ~f:(fun (u, e, post_acts) pre_act ->
          ...
          Preprocess.(ppact.pre_unknowns, ppact.pre_equations, ppact.pre_postprocessing :: post_acts))
    in
    (* Apply the postprocessing after solving. *)
    List.fold postprocessing_actions ~init:(Solve.solve_stratified unknowns' eqns')
  ...
  Either.(
    ...
    | _, First soln ->
        Utils.Log.debug
          Fmt.(
            fun fmt () -> pf fmt "@[<hov 2>Solution found: @;%a@]" (box Solve.pp_partial_soln) soln)
    | _ -> ());
  soln_final
