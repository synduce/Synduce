...
type 'a task = Context.t -> 'a answer Chan.t -> unit -> unit

let first_definite (ctx : Context.t) (tasks : 'a task list) =
  let rec await_first_resp channels =
    let check (chan, sub) =
      match Chan.recv_poll chan with
      | Some (Definite x) ->
        Context.remove_sub ctx ~sub;
        `Fst x
      | Some (Maybe x) ->
        Context.remove_sub ctx ~sub;
        `Snd (Maybe x)
    ...
    then (
      List.iter ~f:(fun (_, sub) -> Context.subkill sub) running;
      term_def)
    else (
      match running with
      | [] ->
  ...
  in
  let start_task i t =
    let sub_ctx = Context.subctx ctx (Fmt.str "task_%i" i) in
