  ...
  let command_of_elts (loc : location) (cn : string) (sl : Annot.t list)
    ...
    | "synth-fun" ->
      (match sl with
       ...
       | _ ->
         Error
           ( loc
           , "synth-fun takes as arguments a function name, variables with sorts a \
              return sort and an optional grammar" ))
    | "synth-inv" ->
      (match sl with
    ...
    | "declare-datatype" ->
      (match sl with
       ...
       | _ ->
         Error
           ( loc
           , "declare-datatypes takes a symbol and a list of constructors as arguments" ))
    | "declare-datatypes" ->
      (match sl with
       | [ a; { sexp = AList b; _ } ] ->
         Ok
    ...
    | "declare-sort" ->
      (match sl with
       ...
       | _ ->
         Error (loc, "A sort declaration with declare-sort takes a name and an index."))
    | "define-fun" ->
      (match sl with
    ...
    | "oracle-assume" ->
      (match sl with
       ...
       | _ ->
         Error
           ( loc
           , "Oracle command oracle-assume doesn't have the right number of arguments." ))
    | "oracle-constraint" ->
      (match sl with
       | [ { sexp = AList q1; _ }; { sexp = AList q2; _ }; body; oname ] ->
         Ok
       ...
       | _ ->
         Error
           ( loc
           , "Oracle command oracle-constraint doesn't have the right number of \
              arguments." ))
    | "declare-oracle-fun" ->
      (match sl with
