...
type index = INum of int | ISym of symbol
...
and sygus_sort = SId of identifier | SApp of identifier * sygus_sort list
...
and sygus_gsterm = GConstant of sygus_sort | GTerm of sygus_term | GVar of sygus_sort
...
let symbol_of_sexp (s : Sexp.t) : symbol =
  match s with Atom symb -> symb | _ -> failwith (Fmt.str "%a is not a symbol" Sexp.pp_hum s)

let sexp_of_symbol (s : symbol) : Sexp.t = Atom s

let index_of_sexp (s : Sexp.t) : index =
  match s with
  | Atom s -> ( match Caml.int_of_string_opt s with Some i -> INum i | None -> ISym s)
  | _ -> failwith "Not an index"

let sexp_of_index (i : index) : Sexp.t =
  match i with INum i -> Atom (Int.to_string i) | ISym s -> Atom s
...
let rec sexp_of_identifier (id : identifier) : Sexp.t =
  ...
  | IdQual (name, sort) -> List [ Atom "as"; sexp_of_symbol name; sexp_of_sygus_sort sort ]

and sygus_sort_of_sexp (s : Sexp.t) : sygus_sort =
  try SId (identifier_of_sexp s)
  with _ -> (
    match s with
...
let literal_of_string (s : string) : literal =
  if String.is_prefix ~prefix:"\"" s then LitString s
  else if String.is_prefix ~prefix:"#x" s then LitHex (String.chop_prefix_exn ~prefix:"#x" s)
...
let literal_of_sexp (s : Sexp.t) : literal =
  match s with Atom atom -> literal_of_string atom | _ -> failwith "not a literal"
...
let rec sygus_term_of_sexp (s : Sexp.t) : sygus_term =
  ...
  | _ -> ( try SyLit (literal_of_sexp s) with _ -> SyId (identifier_of_sexp s))
...
let sygus_gsterm_of_sexp (s : Sexp.t) : sygus_gsterm =
  ...
  | t -> (
      try GTerm (sygus_term_of_sexp t)
      with _ -> failwith (Fmt.str "Not a grammar sygus_term (%a)" Sexp.pp s))
...
let sexp_of_grammar_def (gr : grammar_def) : Sexp.t * Sexp.t =
  ...
  let grammar_decls =
    List.map gr ~f:(fun ((x, y), z) ->
        Sexp.List
          [ sexp_of_symbol x; sexp_of_sygus_sort y; Sexp.List (List.map ~f:sexp_of_sygus_gsterm z) ])
  in
...
let command_of_sexp (s : Sexp.t) : command =
  let command_of_elts (sl : Sexp.t list) : command =
    ...
    | [ Atom "synth-fun"; name; List args; res ] ->
        CSynthFun
          (symbol_of_sexp name, List.map ~f:sorted_var_of_sexp args, sygus_sort_of_sexp res, None)
    | [ Atom com_name; arg1; arg2 ] -> (
        match com_name with
        | "declare-var" -> CDeclareVar (symbol_of_sexp arg1, sygus_sort_of_sexp arg2)
        | "declare-datatype" -> CDeclareDataType (symbol_of_sexp arg1, dt_cons_dec_list_of_sexp arg2)
        | "declare-datatypes" -> (
            match arg2 with
            | List args2 ->
...
let sexp_of_command (c : command) : Sexp.t =
  ...
  | CInvConstraint (a, b, c, d) -> List [ Atom "inv-constraint"; Atom a; Atom b; Atom c; Atom d ]
  ...
  | CSynthInv (name, args, body) -> (
      ...
      | Some body ->
          ...
          List
            [
              Atom "synth-inv"; Atom name; List (List.map ~f:sexp_of_sorted_var args); decls; gramm;
            ]
      | None -> List [ Atom "synth-inv"; Atom name; List (List.map ~f:sexp_of_sorted_var args) ])
  ...
  | CDeclareDataTypes (sortdec, dtdecls) ->
      List
        [
          ...
          List (List.map ~f:(fun x -> Sexp.List (List.map ~f:sexp_of_dt_cons_dec x)) dtdecls);
        ]
  | CDeclareSort (name, id) -> List [ Atom "declare-sort"; Atom name; Atom (Int.to_string id) ]
  ...
  | CSetInfo (sym, lit) -> List [ Atom "set-info"; Atom ":"; Atom sym; sexp_of_literal lit ]
  | CSetLogic sym -> List [ Atom "set-logic"; Atom sym ]
  | CSetOption (sym, lit) -> List [ Atom "set-option"; Atom ":"; Atom sym; sexp_of_literal lit ]

let program_of_sexp_list (sexps : Sexp.t list) : program = List.map ~f:command_of_sexp sexps
...
let is_sat = function RSuccess _ -> true | _ -> false

let is_failed = function RFail -> true | _ -> false

let is_infeasible = function RInfeasible -> true | _ -> false
...
let is_setter_command (c : SyCommand.t) =
  match c with CSetFeature _ | CSetInfo _ | CSetLogic _ | CSetOption _ -> true | _ -> false
...
let is_well_formed (p : program) : bool =
  ...
  match p with
  | [] -> true
  | hd :: tl -> ( match hd with CSetLogic _ -> setter_then_other tl | _ -> setter_then_other p)
...
let max_definition =
  SyCommand.of_sexp (Sexp.of_string "(define-fun max ((x Int) (y Int)) Int (ite (>= x y) x y))")
