...
let simple_ident_of_expr (expr : expression) =
  match expr.pexp_desc with Pexp_ident loc -> simple_ident_of_longident loc.txt | _ -> None
...
let rec fterm_of_expr (expr : expression) : term =
  ...
  | Pexp_apply (fun_or_op, args) -> (
      ...
      | Pexp_ident idnt -> (
          match op_fterm_of_args cur_loc idnt.txt args with Some ft -> ft | None -> default ())
      | _ -> default ())
  ...
  | Pexp_extension (name, payload) ->
      if String.equal name.txt synt_extension then
        match payload with
        | PStr
            [
              {
                pstr_desc = Pstr_eval ({ pexp_desc = Pexp_ident { txt = Lident ident; _ }; _ }, _);
                _;
              };
            ] ->
...
and fterm_of_pattern (pat : pattern) =
  ...
  | _ -> failwith (Fmt.str "Pattern %a not supported in fterm_of_pattern." Pprintast.pattern pat)
