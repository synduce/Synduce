...
type type_decl = TDParametric of id list * id * type_term | TDSimple of id * type_term
...
let rec pp_fterm (frmt : Formatter.t) (t : term) =
  ...
  | FTLet (x, e, body) ->
      Fmt.(pf frmt "let %a = @[<hov 2>%a@]@;in@;@[<hov 2>%a@]" pp_fterm x pp_fterm e pp_fterm body)
  ...
  let rec process_decl d =
    ...
    | SyntObjDecl (_, target_pmrs, spec_id, repr_id) ->
        let rskel_name, d =
          ...
          | d -> (None, d)
        in
        (match rskel_name with Some n -> obj_name := Some (n, spec_id, repr_id) | None -> ());
        d
    | _ -> d
  in
